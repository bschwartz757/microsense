var app = (function() {
  ...
  function readerList({ readers, health }) {
    return readers.map(el => {
      const { status, message } = health.find(
        ind => ind.reader === el.name
      ) || { status: "OK", message: "Stable connection" };
      return {
        name: el.name,
        type: el.type,
        address: el.address,
  ...
  function create_main_fragment$1(component, ctx) {
    ...
    var if_block0 =
      ctx.errorStatusSelected && create_if_block_3(component, ctx);
    ...
    return {
      ...
      p: function update(changed, ctx) {
        ...
        if (
          ...
          button_disabled_value !==
            (button_disabled_value =
              ctx.errorStatusSelected ||
              !ctx.selectedOperation ||
              ctx.submitting)
        ) {
          button.disabled = button_disabled_value;
        }
  ...
  CardBody.prototype._recompute = function _recompute(changed, state) {
    if (changed.readers || changed.health) {
      if (
        this._differs(state.readerList, (state.readerList = readerList(state)))
      )
  ...
  (async function getInitialState() {
    try {
      const [readers, health, operations] = await Promise.all(
        Object.keys(getEndpoints).map(endpoint =>
          apiCall(getEndpoints[endpoint])
        )
      );
      store.set({
        readers: readers,
